...
function fillCoachForm(c) {
  ...
  if (c.passport_url) {
    const img = document.getElementById('coach-passport-preview');
    img.src = c.passport_url; img.style.display = 'block';
    document.getElementById('coach-passport-placeholder').style.display = 'none';
  }
  if (c.doc1_url) {
    const img = document.getElementById('c-doc1-preview');
    img.src = c.doc1_url; img.style.display = 'block';
    document.getElementById('c-doc1-placeholder').style.display = 'none';
  }
  if (c.doc2_url) {
    const img = document.getElementById('c-doc2-preview');
    img.src = c.doc2_url; img.style.display = 'block';
    document.getElementById('c-doc2-placeholder').style.display = 'none';
  }
  if (c.doc3_url) {
    const img = document.getElementById('c-doc3-preview');
    img.src = c.doc3_url; img.style.display = 'block';
    document.getElementById('c-doc3-placeholder').style.display = 'none';
  }
  // populate extra docs gallery
...
function fillWorkerForm(w) {
  ...
  if (w.passport_url) {
    const img = document.getElementById('worker-passport-preview');
    img.src = w.passport_url; img.style.display = 'block';
    document.getElementById('worker-passport-placeholder').style.display = 'none';
  }
  if (w.doc1_url) {
    const img = document.getElementById('w-doc1-preview');
    img.src = w.doc1_url; img.style.display = 'block';
    document.getElementById('w-doc1-placeholder').style.display = 'none';
  }
  if (w.doc2_url) {
    const img = document.getElementById('w-doc2-preview');
    img.src = w.doc2_url; img.style.display = 'block';
    document.getElementById('w-doc2-placeholder').style.display = 'none';
  }
  // populate extra docs gallery
...
async function editCurrentCoach() {
  if (!currentCoachId) return;
  const { data: c } = await sb.from('coaches').select('*').eq('id', currentCoachId).single();
  if (!c) return;
  fillCoachForm(c);
  showPage('add-coach');
  document.getElementById('coach-form-title').textContent = 'Khmbagrel Мarзich';
...
function fillCoachForm(c) {
  ...
  // Build existing docs list from all stored doc URLs (passport_url, doc1_url, doc2_url, doc3_url, extra_docs)
  const existingDocs = [];
  if (c.passport_url) existingDocs.push(c.passport_url);
  if (c.doc1_url)     existingDocs.push(c.doc1_url);
  if (c.doc2_url)     existingDocs.push(c.doc2_url);
  if (c.doc3_url)     existingDocs.push(c.doc3_url);
  if (c.extra_docs && Array.isArray(c.extra_docs)) existingDocs.push(...c.extra_docs);
  renderCoachExtraDocsEdit(existingDocs);
}
...
function renderCoachExtraDocsEdit(urls) {
  ...
  container.innerHTML = coachExtraDocsExisting.map((url, i) => {
    ...
    return `<div class="extra-doc-chip">
      ${isImg
        ? `<img src="${url}" onclick="viewImage('${url}')" title="${name}">`
        : `<span class="extra-doc-icon" onclick="viewImage('${url}')">▤ ${name}</span>`}
...
async function removeCoachExtraDoc(index) {
  ...
  const coachId = document.getElementById('coach-id').value;
  if (coachId) {
    await sb.from('coaches').update({ extra_docs: coachExtraDocsExisting }).eq('id', coachId);
  }
}
...
async function uploadCoachExtraDocs(coachId) {
  ...
  for (const file of input.files) {
    ...
    const path = `${coachId}/extra/${Date.now()}_${Math.random().toString(36).slice(2)}.${ext}`;
    const { error } = await sb.storage.from('coach-docs').upload(path, file, { upsert: true });
    if (error) { console.error('Coach extra doc upload error:', error); continue; }
    const { data: { publicUrl } } = sb.storage.from('coach-docs').getPublicUrl(path);
    newUrls.push(publicUrl);
  }
  if (newUrls.length) {
...
async function removeWorkerExtraDoc(index) {
  ...
  const workerId = document.getElementById('worker-id').value;
  if (workerId) {
    await sb.from('workers').update({ extra_docs: workerExtraDocsExisting }).eq('id', workerId);
  }
}
...
async function uploadWorkerExtraDocs(workerId) {
  ...
  for (const file of input.files) {
    ...
    const path = `${workerId}/extra/${Date.now()}_${Math.random().toString(36).slice(2)}.${ext}`;
    const { error } = await sb.storage.from('worker-docs').upload(path, file, { upsert: true });
    if (error) { console.error('Worker extra doc upload error:', error); continue; }
    const { data: { publicUrl } } = sb.storage.from('worker-docs').getPublicUrl(path);
    newUrls.push(publicUrl);
  }
  if (newUrls.length) {
...
async function editCurrentWorker() {
  if (!currentWorkerId) return;
  const { data: w } = await sb.from('workers').select('*').eq('id', currentWorkerId).single();
  if (!w) return;
  fillWorkerForm(w);
  showPage('add-worker');
  document.getElementById('worker-form-title').textContent = 'Khmbagrel Ashkhat';
...
function fillWorkerForm(w) {
  ...
  // Build existing docs list from all stored doc URLs
  const existingDocs = [];
  if (w.passport_url) existingDocs.push(w.passport_url);
  if (w.doc1_url)     existingDocs.push(w.doc1_url);
  if (w.doc2_url)     existingDocs.push(w.doc2_url);
  if (w.extra_docs && Array.isArray(w.extra_docs)) existingDocs.push(...w.extra_docs);
  renderWorkerExtraDocsEdit(existingDocs);
}
