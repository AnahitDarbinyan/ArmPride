...
// If anything throws an uncaught error anywhere in the app (a bad
// network response, a bug, a dropped connection mid-upload, etc.)
// we show a visible banner instead of the app silently freezing
// with no explanation.
// ============================================================
function showGlobalError(msg) {
  ...
  if (!bar) {
    ...
    bar.style.cssText = 'position:fixed;top:0;left:0;right:0;z-index:99999;background:#c0392b;color:#fff;padding:12px 16px;font-size:14px;text-align:center;box-shadow:0 2px 8px rgba(0,0,0,.3);';
    document.body.appendChild(bar);
  }
  bar.innerHTML = `⚠ ${msg} <span style="text-decoration:underline;cursor:pointer;margin-left:10px" onclick="this.parentElement.remove()">Փակել</span>`;
}

window.addEventListener('error', (e) => {
  console.error('Uncaught error:', e.error || e.message);
  showGlobalError('Առաջացավ անսպասելի սխալ։ Խնդրում ենք ստուգել՝ արդյոք վերջին տվյալները պահվել են, և թարմացնել էջը։');
});

window.addEventListener('unhandledrejection', (e) => {
  console.error('Unhandled promise rejection:', e.reason);
  showGlobalError('Կապի կամ պահպանման խնդիր առաջացավ։ Խնդրում ենք ստուգել վերջին մուտքագրված տվյալները և թարմացնել էջը։');
});

// ============================================================
// SESSION MONITORING
// Without this, an expired/invalidated session fails silently on
// the next save (or, if a shared account is logged out from another
// device, everyone gets kicked with no explanation). This makes it
// visible and sends people back to login with a clear message.
// ============================================================
sb.auth.onAuthStateChange((event, session) => {
  if (event === 'SIGNED_OUT') {
    sessionStorage.setItem('logout-reason', 'Ձեր աշխատաշրջանը ավարտվել է (գուցե դուք դուրս եք եկել այլ սարքից): Խնդրում ենք նորից մուտք գործել։');
    window.location.replace('login.html');
  }
  if (event === 'TOKEN_REFRESHED') {
    console.log('Session token refreshed OK');
  }
});
...
async function loadSchools() {
  ...
  if (error) {
    // Table missing / misconfigured (e.g. 404, or not exposed in API
    // settings) — don't keep hammering it, just fall back to defaults
    // locally so the rest of the app keeps working.
    console.error('Schools table error:', error);
    allSchools = [...DEFAULT_SCHOOLS];
    populateSchoolSelects();
    showGlobalError('Մարզադպրոցների ցուցակը հասանելի չէ (տվյալների բազայի խնդիր)։ Խնդրում ենք տեղեկացնել ադմինիստրատորին։');
    return;
  }
  ...
  if (data && data.length) {
    ...
    allSchools = [...DEFAULT_SCHOOLS];
    const { error: seedErr } = await sb.from('schools').insert(DEFAULT_SCHOOLS.map(name => ({ name })));
    if (seedErr) console.error('Schools seed error:', seedErr);
  }
  populateSchoolSelects();
}
...
async function saveAthlete() {
  ...
  if (athleteId) {
    try {
      ...
      console.error('Post-save step failed:', e);
      btn.innerHTML = originalLabel; btn.disabled = false;
      // The athlete record itself was already saved successfully above —
      // only an attachment/competition step failed — so we say so clearly
      // instead of leaving the button stuck with no explanation.
      err.textContent = 'Մարզիկի հիմնական տվյալները պահվել են, բայց փաստաթղթերի/մրցումների պահման ժամանակ սխալ առաջացավ։ Խնդրում ենք ստուգել և կրկին փորձել։';
      return;
    }
  }
...
async function saveCoach() {
  ...
  if (coachId) {
    try {
      ...
      console.error('Post-save step failed:', e);
      btn.innerHTML = originalLabel; btn.disabled = false;
      err.textContent = 'Մարզիչի հիմնական տվյալները պահվել են, բայց փաստաթղթերի պահման ժամանակ սխալ առաջացավ։ Խնդրում ենք ստուգել և կրկին փորձել։';
      return;
    }
  }
...
async function saveWorker() {
  ...
  if (workerId) {
    try {
      ...
      console.error('Post-save step failed:', e);
      btn.innerHTML = originalLabel; btn.disabled = false;
      err.textContent = 'Աշխատակցի հիմնական տվյալները պահվել են, բայց փաստաթղթերի պահման ժամանակ սխալ առաջացավ։ Խնդրում ենք ստուգել և կրկին փորձել։';
      return;
    }
  }
